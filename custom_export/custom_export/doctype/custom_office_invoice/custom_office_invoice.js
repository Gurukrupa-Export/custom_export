...
frappe.ui.form.on('Custom Office Invoice', {
	exporter: function(frm) {
		frappe.call({
			method: 'custom_export.custom_export.doctype.custom_office_invoice.custom_office_invoice.get_exporter_address',
			args: {
	...
	consignee:function(frm){
		frappe.call({
			method: 'custom_export.custom_export.doctype.custom_office_invoice.custom_office_invoice.get_customer_address',
			args: {
	...
	onload: function(frm) {
		if (frm.doc.docstatus == 0){
			frappe.call({
				method: 'custom_export.custom_export.doctype.gjepc_metal_rate.gjepc_metal_rate.get_gjepc_rate',
				callback: function(r) {
					if (!r.exc) {
			...
			frappe.call({
				method: 'custom_export.custom_export.doctype.gjepc_metal_rate.gjepc_metal_rate.get_gjepc_file',
				callback: function(r) {
					if (!r.exc) {
	...
	get_items_from_packing_list:function(frm){
		...
		erpnext.utils.map_current_doc({
						method: "custom_export.custom_export.doctype.custom_office_invoice.custom_office_invoice.get_items_from_packing_list",
...
frappe.ui.form.on('Description of Goods', {
	bom: function(frm,cdt,cdn) {
		...
		frappe.db.get_value("BOM", {"name": row.bom}, ["quantity","metal_touch","gross_weight","metal_and_finding_weight","diamond_weight","gemstone_weight","total_bom_amount","gold_rate_with_gst","total_bom_amount"], (r)=> {
			if (r){
